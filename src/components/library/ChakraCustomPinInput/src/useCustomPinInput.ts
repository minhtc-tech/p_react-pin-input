...
export interface UsePinInputProps {
  ...
  isInvalid?: boolean;
  /**
   * The regex of values the pin-input should allow
   */
  pattern?: RegExp;
  /**
...
function validate(value: string, pattern: UsePinInputProps["pattern"]) {
  const NUMERIC_REGEX = /^[0-9]+$/;
  const regex = pattern || NUMERIC_REGEX;
...
export function usePinInput(props: UsePinInputProps = {}) {
  ...
  useEffect(() => {
    setValues(toArray(defaultValue) || []);
    // We don't want to listen for updates to `setValues` since it only runs initially
    ...
    (props: InputProps & { index: number }): InputProps => {
      ...
      const onChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (eventValue.length > 2) {
          ...
          if (validate(eventValue, pattern)) {
